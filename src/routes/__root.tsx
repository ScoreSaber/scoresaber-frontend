...
const preloaderStyles = `#app-preloader{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:var(--background,hsl(240 10% 4%));transition:opacity .4s ease,visibility 0s linear .4s}.app-ready #app-preloader{opacity:0;visibility:hidden;pointer-events:none}#app-preloader svg{width:32px;height:32px;color:var(--muted-foreground,hsl(0 0% 55%));opacity:0;animation:app-preloader-in .3s ease .3s forwards,app-preloader-spin 1s linear infinite}@keyframes app-preloader-in{to{opacity:1}}@keyframes app-preloader-spin{to{transform:rotate(360deg)}}@media (prefers-reduced-motion:reduce){#app-preloader{transition:opacity .01s linear,visibility 0s linear .01s}#app-preloader svg{animation:app-preloader-in .01s linear forwards}}`;
